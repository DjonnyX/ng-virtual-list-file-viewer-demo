 ...
 */
export const formatText = (str: string | undefined, options?: IFormatTextOptions) => {
    if (!str) {
        return EMPTY_STRING;
    }
 ...
 * @license Copyright (c) 2026 Evgenii Alexandrovich Grebennikov (djonnyx@gmail.com tg: http://t.me/djonnyx).
 */
const format = (src: string, loading: boolean) => {
    let result = src;
    ...
    if (urls) {
        ...
        for (let i = 0, l = urls.length; i < l; i++) {
            ...
            if (image) {
                ...
                compiledURLs.push([i, (`<span><img${SERVICE_WHITESPACE}src="${image}"${SERVICE_WHITESPACE}class="message-editor-image"${SERVICE_WHITESPACE}draggable="false"${SERVICE_WHITESPACE}width="100%"${SERVICE_WHITESPACE}style="display:block;width:100%;max-height:120px;object-fit:cover;-moz-user-drag:none;-webkit-user-drag:user-select:none;none;border-radius:4px;margin:2px${SERVICE_WHITESPACE}auto;border-radius:6px;"/></span>`), true, index, url.length]);
            }
        }
        if (compiledURLs) {
