...
import { IFileViewerItemParams } from '../item/interfaces';
...
@Component({
  selector: 'x-item-box',
  imports: [CommonModule, ItemComponent, LocaleSensitiveDirective],
  templateUrl: './item-box.component.html',
  styleUrl: './item-box.component.scss'
})
export class ItemBoxComponent implements AfterViewInit {
  ...
  prevData = input<IVirtualListItem<IProxyCollectionItem<IFileViewerItemData>> | null>(null);

  nextData = input<IVirtualListItem<IProxyCollectionItem<IFileViewerItemData>> | null>(null);
  ...
  params: Signal<IFileViewerItemParams>;
  ...
  constructor() {
    ...
    this.classes = computed(() => {
      const params = this.params(), { reseted } = params, initialized = this.initialized(), config = this.config() as any,
        grabbing = config?.grabbing ?? false;
      if (reseted) {
