...
import { IProxyCollectionItem } from '@widgets/file-viewer/file-viewer-items/utils/proxy-collection';
import { IFileViewerItemParams } from './interfaces/store-item-params';
...
export class ItemComponent {
  ...
  data = input<IVirtualListItem<IProxyCollectionItem<IFileViewerItemData>> | null>(null);
  ...
  params = input.required<IFileViewerItemParams>();
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (!!theme) {
        const preset = this._themeService.getPreset(theme.fileViewer.items.item.content);
        if (!!preset) {
    ...
    effect(() => {
      ...
      if (!!config?.focused && !!currentTheme) {
        const preset = this._themeService.getPreset(currentTheme.fileViewer.items.item.styles);
    ...
    effect(() => {
      const theme = this.theme();
      if (!!theme) {
        const preset = this._themeService.getPreset(theme.fileViewer.items.item.content.textEditor.comment);
        if (!!preset) {
    ...
    effect(() => {
      ...
      if (!!containerElement) {
        const preset = this._themeService.getPreset(currentTheme?.fileViewer.items.item.content);
        if (!!preset) {
