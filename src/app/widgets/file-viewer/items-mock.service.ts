...
import { IGetPostsAnswer, IGetPostsData } from './model/posts';
import { IFileViewerItem } from './model/store-item';
...
interface IDB {
    ...
    groups: {
        [groupId: string]: {
            version: number;
            messages?: IVirtualListCollection<IFileViewerItem>;
        }
    };
}
 ...
 * @email djonnyx@gmail.com
 */
const sortByDateTime = (a: IFileViewerItem, b: IFileViewerItem) => {
...
export class ItemsMockService implements ItemsService {
    constructor() { }
    ...
    getPosts(groupId: Id, chunk?: IFileViewerItemsChunkParams): Observable<IGetPostsData> {
        ...
        let listChunk: IVirtualListCollection<IFileViewerItem>;
        if (chunk) {
